...
class Absence(models.Model):
    ...
    approved_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='schedule_app_approved_absences')
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='schedule_app_reported_absences')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class MakeupSession(models.Model):
    ...
    new_room = models.ForeignKey(Room, on_delete=models.CASCADE, related_name='schedule_app_makeup_sessions')
    duration_hours = models.DecimalField(max_digits=3, decimal_places=1, default=1.5)
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='pending')
    notes = models.TextField(blank=True)
    requested_by = models.ForeignKey(User, on_delete=models.CASCADE, related_name='schedule_app_requested_makeups')
    approved_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='schedule_app_approved_makeups')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
